...
while True:
	square.draw()
	win.flip()
	if event.waitKeys(keyList=['left']): #you can also use getKeys, but this is more efficient. Think about why.
		print 'before change', square.size, type(square.size)
		square.size += [10,0]
		print 'after change', square.size
	elif event.waitKeys(keyList=['right']): #you can also use getKeys, but this is more efficient. Think about why.
